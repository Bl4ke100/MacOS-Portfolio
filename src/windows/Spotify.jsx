...
const LiveView = ({ liveTrack, stats }) => {
    ...
    return (
        <div className="pb-32"> {/* Increased bottom padding for scroll clearance */}
            {/* Hero banner */}
            <div
                className="relative flex flex-col px-8 pt-20 pb-8"
                style={{
                ...
                <div className="flex items-end gap-6 w-full">
                    <a href={liveTrack.songUrl} target="_blank" rel="noreferrer" className="flex-shrink-0">
                        <img
                            ...
                            alt={liveTrack.title}
                            className="rounded-lg shadow-2xl hover:scale-105 transition-transform duration-300 object-cover flex-shrink-0"
                            style={{ width: 232, height: 232 }}
                        />
                    </a>
                    <div className="pb-2 flex-1 min-w-0"> {/* Flex-1 and min-w-0 fixes text blowout */}
                        <p className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: '#b3b3b3' }}>Song</p>
                        <h1 className="text-5xl md:text-6xl lg:text-7xl font-black text-white leading-tight mb-2 truncate w-full">{liveTrack.title}</h1>
                        <div className="flex items-center gap-2 mt-2">
                            <img src={liveTrack.albumArt} className="w-6 h-6 rounded-full object-cover shadow-sm flex-shrink-0" alt="" />
                            <p className="text-base font-bold text-white hover:underline cursor-pointer truncate">{liveTrack.artist}</p>
                            <span className="text-[#b3b3b3] text-sm font-medium flex-shrink-0 mx-1">• 2026</span>
                            <span className="text-[#b3b3b3] text-sm font-medium flex-shrink-0">• 3:42</span>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        {stats.tracks.slice(0, 5).map((track, i) => (
                            <div key={i} className="group flex items-center py-2 px-3 hover:bg-white/10 transition-colors cursor-pointer rounded-md">
                                ...
                                <div className="text-sm text-[#b3b3b3] text-right tabular-nums">
                                    {3}:{(i * 12 + 15).toString().padStart(2, '0')}
                                </div>
                            </div>
                        ))}
                    </div>
