        ...
        <div className="flex-1 overflow-y-auto p-6 bg-gradient-to-b from-[#282828] to-[#121212] text-white custom-scrollbar relative">
            {loading ? (
                ...
                <div className="flex flex-col gap-10 animate-in fade-in duration-300 w-full">
                    ...
                    {/* Top Section: Tracks vs Artists Grid */}
                    <div className="grid grid-cols-2 gap-x-10 w-full">
                        {/* On Repeat */}
                        <div className="flex flex-col gap-5">
                            <h4 className="text-[11px] font-bold text-gray-500 uppercase tracking-[0.2em]">On Repeat</h4>
                        ...
                        {/* Top Artists - 3 per row */}
                        <div className="flex flex-col gap-5">
                            <h4 className="text-[11px] font-bold text-gray-500 uppercase tracking-[0.2em]">Top Artists</h4>
                            <div className="grid grid-cols-3 gap-y-8 gap-x-4">
                                {stats?.artists?.slice(0, 6).map((artist, i) => (
                                    <a key={i} href={artist.url} target="_blank" rel="noreferrer" className="flex flex-col items-center gap-2 group">
                                        <div className="w-20 h-20 rounded-full overflow-hidden shadow-xl border border-white/5">
                                            <img src={artist.image} className="object-cover w-full h-full group-hover:scale-110 transition-transform duration-500" alt="" />
                                        </div>
                                        <p className="text-[10px] text-gray-300 font-bold text-center truncate w-full group-hover:text-white transition-colors">{artist.name}</p>
                                    </a>
                                ))}
                            </div>
                        </div>
                    ...
                    {/* Recently Played - Rectangular Cards */}
                    <div className="pb-10 w-full">
                        <h4 className="text-[11px] font-bold text-gray-500 uppercase tracking-[0.2em] mb-6">Recently Played</h4>
                        <div className="grid grid-cols-2 gap-4 w-full">
                            {stats?.recentlyPlayed?.slice(0, 4).map((track, i) => (
                                <div key={i} className="flex items-center gap-4 bg-white/5 p-4 rounded-xl border border-white/5 hover:bg-white/10 transition-all cursor-default group">
                                    <img src={track.cover} className="w-12 h-12 rounded shadow-lg flex-shrink-0" alt="" />
                                    <div className="min-w-0 flex-1 px-1">
                                        <p className="text-[12px] font-bold text-white truncate group-hover:text-green-400 transition-colors">{track.title}</p>
